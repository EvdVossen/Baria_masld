...
met_align_normal_ballooning <- as.data.frame(rownames(bval)) %>%
  setNames("Sample") %>% 
  ...
  dplyr::filter(histology %in% c("Normal", "Ballooning")) %>% 
...
bval_normal_ballooning <- bval %>% dplyr::filter(rownames(.) %in% met_align_normal_ballooning$Sample) %>% t(.)
...
coMethRegpath_norm_ball <- "Intermediate_files/DMR/normal_ballooning/"
dir.create(coMethRegpath_norm_ball, recursive = T)

# residuals -> to remove covariate effects from methylation values by fitting probe-specific linear models
# resid_obj <- GetResiduals(bval_normal_ballooning,
#                           betaToM = T,
#                           pheno_df = met_align_normal_ballooning,
#                           covariates_char = c("age", "sex", "age_sq"))
# saveRDS(resid_obj, paste0(coMethRegpath_norm_ball, "Residuals_object.RDS"))
resid_obj <- readRDS(paste0(coMethRegpath_norm_ball, "Residuals_object.RDS"))
...
#                               output = "CpGs",
#                               nCores_int = 10)
# saveRDS(coMethReg, paste0(coMethRegpath_norm_ball, "cometh_residobj_norm_nash_gene.RDS"))
coMethReg <- readRDS(paste0(coMethRegpath_norm_ball, "cometh_residobj_norm_nash_gene.RDS"))
...
#                               output = "CpGs",
#                               nCores_int = 10)
# saveRDS(coMethReg_ig, paste0(coMethRegpath_norm_ball, "cometh_residobj_norm_nash_intergene.RDS"))
coMethReg_ig <- readRDS(paste0(coMethRegpath_norm_ball, "cometh_residobj_norm_nash_intergene.RDS"))

#for the gene part
# system.time(
#   out_df <- lmmTestAllRegions(
#     betas = bval_normal_ballooning,
#     region_ls = coMethReg,
#     pheno_df = met_align_normal_ballooning,
...
#   )
# )
# rio::export(out_df, paste0(coMethRegpath_norm_ball, "Result_coMethreg_gene_normal_ballooning.xlsx"))
out_df <- rio::import(paste0(coMethRegpath_norm_ball, "Result_coMethreg_gene_normal_ballooning.xlsx"))
...
#   )
# )
# rio::export(out_df_ig, paste0(coMethRegpath_norm_ball, "Result_coMethreg_intergene_normal_ballooning.xlsx"))
out_df_ig <- rio::import(paste0(coMethRegpath_norm_ball, "Result_coMethreg_intergene_normal_ballooning.xlsx"))
...
rio::export(df, paste0(coMethRegpath_norm_ball, "Result_coMethDMR_Fdr005_normal_ballooning.xlsx"))
